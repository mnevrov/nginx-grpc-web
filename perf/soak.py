...
class SoakPolicy:
    ...
    def validate(self) -> None:
        ...
        if self.warmup_seconds >= self.min_duration_seconds:
            raise ValueError("warmup_seconds must be smaller than min_duration_seconds")
        if self.max_rss_slope_mib_per_hour < 0:
            raise ValueError("max_rss_slope_mib_per_hour must be >= 0")
        if self.max_rss_growth_mib < 0:
            raise ValueError("max_rss_growth_mib must be >= 0")
...
def linear_slope_per_hour(points: list[tuple[float, float]]) -> float:
    """Least-squares slope for y(MiB) over x(seconds), returned as MiB/hour."""
    if len(points) < 2:
...
def evaluate_soak(timeline: dict[str, Any], policy: SoakPolicy) -> dict[str, Any]:
    policy.validate()
    samples = sorted(
        [dict(item) for item in timeline.get("samples", [])],
    ...
    rss_growth = 0.0
    peak_rss = 0.0
    peak_memory_current = 0.0
    if samples:
        peak_rss = max(float(item["rss_mib"]) for item in samples)
        peak_memory_current = max(float(item.get("memory_current_mib", 0.0)) for item in samples)
    ...
    final_probe = _event_dict(events, "final_probe")
    if not bool(final_probe.get("success")):
    ...
    # Shared CI validates the harness but must never be presented as production soak evidence.
    evidence_class = "controlled" if strict else "harness_only"
...
def render_markdown(result: dict[str, Any]) -> str:
    ...
    for name in ("steady", "churn", "cancel", "backend_restart", "transport_reset", "final_probe", "nginx"):
    ...
    if result["reasons"]:
        ...
        lines.append("- all configured soak gates passed")

    lines.extend(
        [
            "",
            "`harness_only` output validates orchestration only. A production soak claim requires strict mode, the configured minimum duration/sample count, and all lifecycle/memory gates to pass.",
            "",
        ]
    )
    return "\n".join(lines)
